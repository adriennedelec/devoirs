...
export function sha256Hex(value: string) {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    for (let index = 0; index < 16; index += 1) words[index] = readUint32(padded, offset + index * 4);
...
export function hashLegacyHexPasswordForStorage(password: string) {
  const hex = Array.from(password).map((char) => char.charCodeAt(0).toString(16).padStart(2, '0')).join('');
  return `sha256:${hex}`;
}
