...
describe('Profil et modules branchés sur la base activité', () => {
  ...
  it('alimente les graphiques et l’historique Profil depuis les vraies activités stockées', async () => {
    ...
    const emmaStars = within(within(profileList).getByRole('article', { name: /profil de emma/i })).getByLabelText(/étoiles collectées par emma/i);
    expect(emmaStars).toHaveTextContent(/^16$/);
    expect(emmaStars).not.toHaveTextContent(/étoiles collectées/i);
    const louaneStars = within(within(profileList).getByRole('article', { name: /profil de louane/i })).getByLabelText(/étoiles collectées par louane/i);
    expect(louaneStars).toHaveTextContent(/^6$/);
