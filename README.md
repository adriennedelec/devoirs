...
Évolution authentification provisoire : l’application affiche maintenant une page `Connexion administrateur` avant le shell enfant. Pour ce premier lot, un seul compte administrateur local est accepté, la session est conservée dans `sessionStorage`, puis l’utilisateur connecté `admin` apparaît en bas du menu latéral gauche. Le reste du site reste inchangé après connexion.
