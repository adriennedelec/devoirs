...
Évolution dictée validée ensuite : le module `Dictée magique` propose maintenant deux modes explicites, **Dictée de mots** et **Dictée normale**. La dictée de mots est le mode prioritaire : le parent saisit une série de mots, choisit un ou plusieurs temps verbaux (`Présent`, `Imparfait`, `Passé composé`, `Futur`), puis l’app génère un **texte court contenant tous les mots**. Le texte généré reste **masqué par défaut pour l’élève** ; l’action principale côté enfant est `Lire le texte à l’élève`, avec une option `Afficher pour le parent` uniquement pour préparer/vérifier.
