...
Évolution dictée validée ensuite : le module `Dictée magique` propose maintenant deux modes explicites, **Dictée de mots** et **Dictée normale**. La dictée de mots est le mode prioritaire et utilise toute la largeur disponible à droite du menu latéral : le parent saisit une série de mots ou les importe via **fichier/photo** ; un flux OCR API-ready détecte automatiquement les mots et remplit le champ `Série de mots`. La saisie est découpée en mots distincts dès qu’un séparateur apparaît (espace, retour ligne, virgule, point, slash, tiret, etc.), puis chaque mot est comparé à un dictionnaire français embarqué d’environ **336 000 mots** (`an-array-of-french-words`), derrière une couche service remplaçable par API. Si un mot saisi ou OCRisé paraît inconnu, une carte **Mot à confirmer** demande au parent de vérifier/valider avant de continuer. Le parent choisit ensuite un ou plusieurs temps verbaux (`Présent`, `Imparfait`, `Passé composé`, `Futur`), puis l’app peut générer une **mini-histoire courte, naturelle, logique et adaptée au primaire** avec deux moteurs : **`IA locale Ollama` par défaut** via le proxy Vite `/api/ollama/generate` vers `http://127.0.0.1:11434` (`llama3.1:8b`), ou `Local secours` (déterministe, rapide, limité). Le moteur Ollama reçoit un prompt professoral et le service valide ensuite automatiquement que tous les mots sont présents **une seule fois chacun** ; si le modèle oublie ou répète un mot, l’app relance jusqu’à obtenir un texte conforme. Si Ollama échoue encore après les relances, le service bascule sur un texte de secours validé pour ne pas bloquer le parent. Pendant l’attente, le bouton affiche `Génération en cours` avec trois points animés. Le texte généré reste **masqué par défaut pour l’élève** ; l’action principale côté enfant est `Lire le texte à l’élève`, avec une option `Afficher pour le parent` uniquement pour préparer/vérifier.
