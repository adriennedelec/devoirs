...
Évolution dictée validée ensuite : le module `Dictée magique` propose maintenant deux modes explicites, **Dictée de mots** et **Dictée normale**. La dictée de mots est le mode prioritaire et utilise toute la largeur disponible à droite du menu latéral : le parent saisit une série de mots ou les importe via **fichier/photo** ; un flux OCR API-ready détecte automatiquement les mots et remplit le champ `Série de mots`. La saisie est découpée en mots distincts dès qu’un séparateur apparaît (espace, retour ligne, virgule, point, slash, tiret, etc.), puis une validation légère côté client repère les formes manifestement suspectes sans embarquer de dictionnaire volumineux dans le bundle. Si un mot saisi ou OCRisé paraît inconnu, une carte **Mot à confirmer** demande au parent de vérifier/valider avant de continuer. Le parent choisit ensuite un ou plusieurs temps verbaux (`Présent`, `Imparfait`, `Passé composé`, `Futur`), puis l’app génère une **mini-histoire courte, naturelle, logique et adaptée au primaire** via OpenAI côté serveur. Il n’y a plus d’option `Local secours`, jugée trop mauvaise pour être exploitable. Le modèle reçoit un prompt professoral et le service valide ensuite automatiquement que tous les mots sont présents **une seule fois chacun** ; si le modèle oublie ou répète un mot, l’app relance jusqu’à trois fois avec les erreurs détectées, puis affiche une erreur explicite si le modèle n’atteint toujours pas le contrat. Pendant l’attente, le bouton affiche `Génération en cours` avec trois points animés. Le texte produit est affiché directement au parent, puis les contrôles de lecture / checklist des mots apparaissent dessous. La lecture enfant propose maintenant trois vitesses (`Rapide` = vitesse historique, `Moyen` = 3 fois plus lent, `Lent` = 6 fois plus lent) et la zone de réponse affiche en temps réel le nombre de mots écrits par l’enfant.

Évolution Lecture validée ensuite : la page `Lecture` devient un atelier complet en 4 blocs : génération IA d’une histoire à partir de `personnage`, `animal`, `objet`, `lieu` et `taille XS/S/M/L/XL/XXL`; édition du prompt avec placeholders `{{personnage}}`, `{{animal}}`, `{{objet}}`, `{{lieu}}`, `{{taille}}`; affichage du texte avec bouton `Démarrer l’enregistrement` et chronomètre; puis analyse de la transcription avec mots en erreur colorés, mots/minute, temps total, nombre d’erreurs et précision. Le flux conserve le bonus compréhension existant pour ne pas casser les tests et usages précédents.

Évolution authentification : l’application affiche une page `Connexion administrateur` avant le shell enfant. Le compte administrateur est validé côté serveur via `/api/auth/login`, avec session `HttpOnly`, et les endpoints sensibles (`/api/family-database`, `/api/openai/generate`) exigent cette session. La fiche `Famille` peut toujours définir un identifiant `Utilisateur` et un `Mot de passe` pour un accès famille local avec rôle simple `Utilisateur`, jamais `Administrateur`.

Variables d’environnement nécessaires en production Vercel :

```txt
DEVOIRS_ADMIN_USERNAME=admin
DEVOIRS_ADMIN_PASSWORD=<mot-de-passe-admin-fort>
DEVOIRS_SESSION_SECRET=<secret-long-aléatoire>
OPENAI_API_KEY=<clé-openai-si-génération-ia-active>
