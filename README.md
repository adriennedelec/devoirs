...
Évolution authentification provisoire : l’application affiche maintenant une page `Connexion administrateur` avant le shell enfant. Un compte administrateur local reste accepté, la session est conservée dans `sessionStorage`, puis l’utilisateur connecté apparaît en bas du menu latéral gauche avec son rôle. La fiche `Famille` permet aussi de définir un identifiant `Utilisateur` et un `Mot de passe` pour un accès famille : ces identifiants connectent avec le rôle simple `Utilisateur`, jamais avec le rôle `Administrateur`. Le reste du site reste inchangé après connexion.
